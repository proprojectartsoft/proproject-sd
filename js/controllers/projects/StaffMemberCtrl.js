...
function StaffMemberCtrl($rootScope, $scope, $state, $filter, $ionicModal, $stateParams, SiteDiaryService, SettingService, $indexedDB) {
    ...
    if ((!(vm.diaryId === false) && !(vm.index === 'create')) || !(isNaN(vm.index))) {
        vm.local.data = {
            ...
            hourly_rate_formated: (localStorage.getObject('currency')) || '') + " " + vm.create.site_attendance.staffs[vm.index].hourly_rate && (vm.create.site_attendance.staffs[vm.index].hourly_rate
        }
